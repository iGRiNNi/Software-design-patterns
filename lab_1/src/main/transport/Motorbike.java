...
public class Motorbike implements Transportable {
    ...
    /*
    public Model getModelsNames(){
        Model q = new Model();
        q = head;
        Model p = head.next;
        while(p!=head){
            q.next.nameModel = p.nameModel;
            p = p.next; q = q.next;
        }
        return q;
    }
    ...
    public double modelPriceByName(String nameModel) throws NoSuchModelNameException {       //метод для получения значения цены модели по её названию
        boolean flag = true;
        Model p = head.next;            //нужна ли провверка на head null???
        double price = 0;            //null или вовсе не присваивать значение???
        while (flag && p != head) {
    ...
    public boolean equals(Object obj) {
        ...
        /*
        if (!(obj instanceof Transportable)) return false;                          // Сравнивает Transportable.
        Transportable objMoto = (Transportable) obj;
        if (!(this.brand.equals(objMoto.getBrand()))) return false;
        if (this.getModelArraySize() != objMoto.getModelArraySize()) return false;
        double[] prices = objMoto.getModelsPrice();
        String[] names = objMoto.getModelsNames();
        Model p = head.next;
        for (int i = 0; i < this.getModelArraySize(); i++) {
            if (!(p.nameModel.equals(names[i]) && p.priceModel == prices[i])) return false;
            p = p.next;
        }
        return true;
        ...
        if (this != obj) {
            if (!(obj instanceof Motorbike)) {
                ...
                Motorbike objMoto = (Motorbike) obj;
                if (!(this.brand.equals(objMoto.getBrand()))) {
                    result = false;
                } else if (this.getModelArraySize() != objMoto.getModelArraySize()) {
                    result = false;
                } else {
                    double[] prices = objMoto.getModelsPrice();
                    String[] names = objMoto.getModelsNames();
                    Model p = head.next;
                    for (int i = 0; i < this.getModelArraySize(); i++) {
                        if (!(p.nameModel.equals(names[i]) && p.priceModel == prices[i])) {
                            result = false;
                            break;
                        }
                        p = p.next;
...
//        return true;
    ...
    public Object clone() throws CloneNotSupportedException {
        ...
        result.head.next = result.head;
        result.head.prev = result.head;
        Model res = result.head;
        //Model res = new Model(p.nameModel, p.priceModel);
        //result
        while (p != head) {

            Model q = new Model(p.nameModel, p.priceModel);
            q.next = res.next;
            q.prev = res;
            res.next.prev = q;
            res.next = q;
